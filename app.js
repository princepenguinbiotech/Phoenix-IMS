...
function getAuditEvents() {
  ...
  state.consumptions.forEach((entry) => {
    ...
    const count = Math.abs(toNumber(entry.inputCount) || 0);
    const unitsPerSlide = toNumber(entry.unitsPerSlide);
    const slideUnitName = getUnitName(entry.slideUnitId);
    const note = mode === "slides"
      ? `${count || Math.abs(qtyDelta / (unitsPerSlide || 1))} slide(s)${unitsPerSlide > 0 ? ` (${Math.abs(qtyDelta)} ${slideUnitName})` : ""}`
...
function renderActivity() {
  ...
  state.consumptions.forEach((consumption) => {
    ...
    const count = Math.abs(toNumber(consumption.inputCount) || 0);
    ...
    const amountLabel = mode === "slides"
      ? `${count || Math.abs(qty / (unitsPerSlide || 1))} slide(s)${unitsPerSlide > 0 ? ` (${Math.abs(qty)} ${slideUnitName})` : ""}`
      : `${Math.abs(qty)}`;
...
consumptionList.addEventListener("click", async (event) => {
  ...
  const stockChange = slideBased ? inputCount * unitsPerSlide : inputCount;
